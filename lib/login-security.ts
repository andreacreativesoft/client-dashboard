...
// TEMP (dev/démo) : neutralise le rate limiting login + reset password.
// Passer à `false` pour réactiver les garde-fous de sécurité.
const RATE_LIMIT_DISABLED: boolean = true;
...
function recordFailedLoginMemory(email: string): void {
    ...
    // If phase 2 and hit the limit → block account in DB
    if (entry.phase === 2 && entry.count >= LOGIN_MAX_ATTEMPTS_PER_PHASE) {
        blockAccount(email).catch((err) => console.error("Failed to block account:", err));
    }
}
