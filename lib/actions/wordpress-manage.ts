...
export async function getWordPressStatus(websiteId: string): Promise<{
  ...
  // Check for full credentials record (created via Connect WordPress form)
  ...
  // No wordpress_credentials record, but integration exists
  // (connected via Integrations tab — simple flow).
  // Still report as connected; credentials are in the integrations table.
