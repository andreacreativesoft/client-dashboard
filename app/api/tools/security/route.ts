...
export async function POST(request: NextRequest) {
  ...
  const { websiteId, checks: requestedChecks } = body as { websiteId: string; checks?: string[] };

  if (!websiteId) {
    return NextResponse.json({ error: "websiteId required" }, { status: 400 });
  }

  // If specific checks requested, validate them
  const singleCheckMode = requestedChecks && requestedChecks.length > 0;
  ...
  // Only create a DB record for full audits, not single retests
  ...
  if (!singleCheckMode) {
    ...
    checkId = (checkRow as { id: string } | null)?.id || null;
  ...
  try {
    ...
    const shouldRun = (name: string) => !singleCheckMode || requestedChecks!.includes(name);

    // Fetch homepage (needed for most checks)
    ...
    if (shouldRun("X-Content-Type-Options")) {
    ...
    if (shouldRun("Content-Security-Policy")) {
    ...
    if (shouldRun("Permissions-Policy")) {
    ...
    if (shouldRun("WordPress Version Exposed")) {
    ...
    // 12. WP Login page
    if (shouldRun("Login Page Exposed")) {
    ...
    if (shouldRun("REST API User Listing")) {
    ...
    // 15. wp-config.php
    if (shouldRun("wp-config.php Exposed")) {
    ...
    // 16. readme.html
    if (shouldRun("readme.html Exposed")) {
    ...
    if (shouldRun("PHP Errors Visible")) {
    ...
    if (shouldRun("Directory Listing")) {
    ...
    // Only update the DB record for full audits
    if (!singleCheckMode && checkId) {
      await admin
        .from("site_checks")
        .update({
    ...
    return NextResponse.json({
      success: true,
      checkId: singleCheckMode ? undefined : checkId,
      score: singleCheckMode ? undefined : score,
      summary: singleCheckMode ? undefined : summary,
      results,
      duration,
      singleCheck: singleCheckMode || false,
    });
  } catch (e) {
    const duration = Date.now() - startTime;
