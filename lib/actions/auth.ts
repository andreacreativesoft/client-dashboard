...
export async function loginAction(
    ...
    if (recaptchaToken) {
        ...
        return {
            success: false,
            error: "Security verification required. Please reload the page and try again.",
        };
    }
    ...
    // 4. Success — clear attempts + record last login
...
export async function forgotPasswordAction(
    ...
    const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo:
            redirectUrl || `${process.env.NEXT_PUBLIC_APP_URL}/auth/callback?next=/settings`,
    });
